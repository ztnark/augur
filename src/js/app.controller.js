 ...
 * Author: priecint
 */
define(function AppControllerModule(require) {
    "use strict";

    return ["$rootScope", "$state", function AppController($rootScope, $state) {
        var vm = this;

        /**
         * Takes care of forward to correct state.
        ...
        $rootScope.$on("$stateChangeStart", function (event, toState, toParams, fromState, fromParams) {
            console.log("AppController.js[$stateChangeStart]: %o, %o, %o, %o, %o", event, toState, toParams, fromState, fromParams);
            var toStateName = toState.name;
            if (_shouldRedirectToMobileState(toStateName)) {
                event.preventDefault();
                $state.go(toStateName + ".mobile", toParams);
            }
        ...
        $rootScope.$on('$stateChangeError', function (event, toState, toParams, fromState, fromParams, error) {
            console.error("AppController.js[$stateChangeError]: %o, %o, %o, %o, %o, %o", event, toState, toParams, fromState, fromParams, error, error.stack);
        });
